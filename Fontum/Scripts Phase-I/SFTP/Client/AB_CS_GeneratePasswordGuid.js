...
define(['N/log', 'N/currentRecord', 'N/record', 'N/ui/dialog', 'N/search','N/https','N/url','N/runtime'], function (log, currentRecord, record, dialog, search,https,urlmodule,runtime) {
    ...
    function generateGUID(e) {
        ...
        try {
            ...
            var searchlookup = search.lookupFields({
                ...
                id: rec.id,
                columns: ['custrecord_ab_hostname','custrecord_ab_connection_script']
            });
            log.debug(title + 'searchlookup', searchlookup);
            var hostname = searchlookup.custrecord_ab_hostname;
            var scriptsObject = searchlookup.custrecord_ab_connection_script;
 ...
 //sandbox    
           // var popUp = window.open('/app/site/hosting/scriptlet.nl?script=913&deploy=1&hostname=' + hostname+ '&recId='+rec.id+ '&scriptsRestriction='+scriptsRestriction, 'SFTP GUID', 'width=1200px,height=600px');
    ...
    function sftpConnection(){
        ...
        var requiredFieldArray = [];
         try{
            ...
            var passwordGuid= config.getValue('custrecord_ab_password_guid');
            var username= config.getValue('custrecord_ab_username');
            var url= config.getValue('custrecord_ab_hostname');
            var port= config.getValue('custrecord_ab_port');
            var hostKey= config.getValue('custrecord_ab_hostkey');
            var inboundDirectory= config.getValue('custrecord_ab_inbound_path');
            var outboundDirectory= config.getValue('custrecord_ab_outbound_path');
            if(!passwordGuid){
            ...
            if(!inboundDirectory){
                requiredFieldArray.push("SERVER INBOUND DIRECTORY PATH");
            }
            if(!outboundDirectory){
                requiredFieldArray.push("SERVER OUTBOUND DIRECTORY PATH");
            }
            if(requiredFieldArray.length){
                ...
                var suiteletURL = urlmodule.resolveScript({
                    scriptId: 'customscriptab_sl_testconnection',
