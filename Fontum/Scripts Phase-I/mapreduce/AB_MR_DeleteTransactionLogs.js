    ...
    * File:        AB_MR_DeleteTransactionLogs.js
    * Date:        07/04/2022
    *
    *
    ***********************************************************************/
...
define(['N/log','N/record','N/runtime','../dao/AB_CLS_OpenTransactionLogs.js','N/search'], function(log,record,runtime,CLS_TransactionLogs,search) {
    /**
     * @param  {} filter
     * @param  {} operator
     ...
     * Details : Fetch transaction log record that is are going to be deleted
     */
    function getInputData() {
        var title = 'getInputData()::';
         try{
            var transactionLogsRecords;
            var scriptObj = runtime.getCurrentScript();
            var dateOperator = scriptObj.getParameter({name: 'custscript_ab_operator'});
            var datePeriod = scriptObj.getParameter({name: 'custscript_ab_period'});
            dateOperator = getfilterFormate(dateOperator,true);
            datePeriod =  getfilterFormate(datePeriod,false);
            transactionLogsRecords = CLS_TransactionLogs.getTransactionLog(dateOperator,datePeriod);
          
            }catch(error){
                log.error(title+error.name,error.message);
        } 
        return transactionLogsRecords;
    }
    ...
    function map(context) {
        ...
        var delId ;
         try{
            ...
            delId = CLS_TransactionLogs.deleteRec(recObj.id);
