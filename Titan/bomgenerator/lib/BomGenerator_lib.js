...
define(['N/search', 'N/record', 'N/https', 'N/runtime'],
    function (nsSearch, nsRecord, nsHttps, runtime) {
         ...
         */
        function getBOMTemplates(subsidiaryId, locationId) {
            var output = [], outputLine;
            var transactionSearchObj = nsSearch.create({
                type: "customtransaction_ab_createbomtemplate",
                filters:
                    [
                        ["subsidiary", "anyof", subsidiaryId],
                        "AND",
                        ["custbody_ab_location", "anyof", locationId],
                        "AND",
                        ["status", "anyof", "Custom106:A"], //Active
                        "AND",
                        ["mainline", "is", "T"]
                    ],
                columns: [nsSearch.createColumn({name: "custbody_ab_title", label: "Title"})]
            });

            transactionSearchObj.run().each(function (result) {
                outputLine = {};
                outputLine.id = result.id;
                outputLine.name = result.getValue({"name": "custbody_ab_title"});
        ...
        return {
            ...
            getBOMTemplates: getBOMTemplates
        };
    });
