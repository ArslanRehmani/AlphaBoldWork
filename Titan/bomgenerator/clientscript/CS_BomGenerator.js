...
define(['N/ui/message', 'N/currentRecord', 'N/search', 'N/record', 'N/https', '../lib/BomGenerator_lib.js'],
    function (uiMessage, currentRecord, nsSearch, nsRecord, nsHttps, library) {
        ...
        function pageInit(context) {
            ...
            populateTemplates(context.currentRecord);
        }
        ...
        function fieldChanged(context) {
            ...
            if (fieldName === 'custpage_config') {
                ...
                if (locations.length > 0) {
                    ...
                    populateTemplates(currentRecord);
                }

                var subsidiaryId = currentRecord.getValue({"fieldId": "custpage_subsidiary"});
                // TODO : Set Default Currency from Subsidiary
                var subsidiaryCols = nsSearch.lookupFields({type:"subsidiary",id:subsidiaryId,columns:["currency"]});
                currentRecord.setValue({fieldId:"custpage_currency",value:subsidiaryCols.currency[0].value});
            ...
            } else if (fieldName === 'custpage_location') {
                debugger;
                populateTemplates(currentRecord);
            } else if (fieldName == "custpage_template") {
                fillTemplateData(currentRecord);
            }
        }
         ...
         */
        function populateTemplates(currentRecord) {
            ...
            if (!!subsidiary && !!location) {
                var templtes = library.getBOMTemplates(subsidiary, location);
                var templateFLD = currentRecord.getField({"fieldId": "custpage_template"});
                templateFLD.removeSelectOption(null);
                if (!!templtes && templtes.length > 0) {
                    templtes.unshift({id: "", name: "-BLANK-"});
                    library.insertSelectOPtions(templateFLD, templtes);
                }
            }
        }
         ...
         * @param {object} currentRecord
         */
        function fillTemplateData(currentRecord) {
            var templateId = currentRecord.getValue({"fieldId": "custpage_template"});
            if (!!templateId) {
                var nsRec = nsRecord.load({type: "customtransaction_ab_createbomtemplate", id: templateId});
                var lineCount = nsRec.getLineCount({sublistId: "line"});
                var templateLineItem, templateLineQty, templateLineYield, templateLinePartCost, templateLineSource;

                for (var i = 0; i < lineCount; i++) {
                    templateLineItem = nsRec.getSublistValue({
                        sublistId: "line",
                        fieldId: "custcol_bomtemplateitem",
                        line: i
                    });
                    templateLineQty = nsRec.getSublistValue({
                        sublistId: "line",
                        fieldId: "custcol_ab_bomtemplateqty",
                        line: i
                    });
                    templateLineYield = nsRec.getSublistValue({
                        sublistId: "line",
                        fieldId: "custcol_ab_bomtemplateyield",
                        line: i
                    });
                    templateLinePartCost = nsRec.getSublistValue({
                        sublistId: "line",
                        fieldId: "custcol_ab_bomtemplatepartcost",
                        line: i
                    });
                    templateLineSource = nsRec.getSublistValue({
                        sublistId: "line",
                        fieldId: "custcol_ab_bomtemplateitemsrc",
                        line: i
                    });
                    ...
                    currentRecord.setCurrentSublistValue({
                        sublistId: "custpage_sublist",
                        fieldId: "custpage_item",
                        value: templateLineItem
                    });
                    currentRecord.setCurrentSublistValue({
                        sublistId: "custpage_sublist",
                        fieldId: "custpage_qty",
                        value: templateLineQty
                    });
                    currentRecord.setCurrentSublistValue({
                        sublistId: "custpage_sublist",
                        fieldId: "custpage_yield",
                        value: templateLineYield
                    });
                    currentRecord.setCurrentSublistValue({
                        sublistId: "custpage_sublist",
                        fieldId: "custpage_partcost",
                        value: templateLinePartCost
                    });
                    currentRecord.setCurrentSublistValue({
                        sublistId: "custpage_sublist",
                        fieldId: "custpage_item_source",
                        value: templateLineSource
                    });
        ...
        function commitItem(currentRecord, doNotShowMessage, doNotReset) {
            try {
                ...
                if (!validatePriceInfo(currentRecord)) return false;
         ...
         * @param {object} currentRecord
         */
        function validatePriceInfo(currentRecord) {
            var result = true;
            ...
            var priceLevel = currentRecord.getValue({
                fieldId: 'custpage_pricelevel',
            });


            if (rate == null || rate == "") {
                result = confirm("Do you want to continue without price setting ?");
            } else if (!currency || !priceLevel) {
                alert("Please choose currency and price level for rate.");
                result = false;
            }
        ...
        function createBOMJSON(currentRecord) {
            ...
            obj.priceInfo = [];
            obj.priceInfo.push({
                "currency": currentRecord.getValue({fieldId: "custpage_currency"}),
                "priceLevel": currentRecord.getValue({fieldId: "custpage_pricelevel"}),
                "rate": currentRecord.getValue({fieldId: "custpage_rate"})
            });
